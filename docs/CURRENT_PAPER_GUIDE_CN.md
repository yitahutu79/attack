...
- `ALoRa` 已补齐 `W=128` 统一口径结果，当前主文与汇报页可以直接采用这组新值。
...
- 是否继续保留结果表里的 `W` 列。现在 `ALoRa` 已补成 `W=128`，但是否删列仍取决于你想不想在文中保留“外部方法输入长度透明度”。
...
2. 表格口径说明  
   现在 `ALoRa` 已补成 `W=128`，但 `UNSW` 页仍混有 `target_fpr=0.15` 主结果和部分 `0.05` 外部 fair 参考值，文字说明必须跟上。
...
1. 决定是否“全表统一去掉 W 列”：  
   只有在窗口长度完全统一后才建议去掉。  
2. 固化最终表格与文字解释（尤其 target FPR 口径说明）。  
3. 最后再做语言润色与版式收尾。
